  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (titleRef.current && portraitRef.current) {
        gsap.utils.toArray('.parallax-title').forEach((element: any) => {
          gsap.fromTo(
            element,
            { y: 100, opacity: 0 },
            {
    ...
    >
      ...
      <div className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute bottom-20 left-12 z-20">
          ...
          <h3 className="font-display text-4xl text-white">Mo Muscle</h3>
          <p className="font-mono-custom text-sm text-[#0582c0]">The Mind Is Primary</p>
        </div>
      </div>
    </section>
  );
